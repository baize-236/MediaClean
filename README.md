...
一款本地运行的开源图片与视频去水印工具。手动框选水印，使用 AI 修复、快速填补或区域模糊处理，支持批量任务和处理前后对比。

**图片去水印 · 视频去水印 · 批量处理 · 本地运行**

- **图片去水印**：支持矩形、画笔和橡皮擦选区，使用 LaMa 补全水印背景。
- **视频固定水印处理**：框选固定位置，使用快速填补或区域模糊。
- **视频移动水印去除（实验）**：手动选择水印模板，检查跟踪轨迹后逐帧修复。

AI 修复是去水印的处理方式，并不保证恢复被遮挡的原始细节；区域模糊属于遮盖，移动水印可能跟丢或产生闪烁。

> 当前面向 Windows，界面为简体中文。这是本地浏览器应用，不是在线服务或独立安装器。视频 AI 修复仍为实验功能。

**单用户模式，无需登录和额度。** Windows 原生启动见下文；Linux 和 Docker 用户请查看 [本地部署指南](docs/LOCAL_DEPLOYMENT.md)。
